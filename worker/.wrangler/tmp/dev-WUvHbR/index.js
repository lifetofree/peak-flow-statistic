...
// .wrangler/tmp/bundle-vRRkzo/checked-fetch.js
...
var DatabaseClient = class {
  ...
  async updateOne(table, filter, data) {
...
// src/routes/zone.ts
function calculateZone(bestReading, personalBest) {
...
__name(calculateZone, "calculateZone");

// src/routes/user.ts
var app2 = new Hono2();
var PAGE_SIZE = 20;
var validateShortLink = /* @__PURE__ */ __name(async (c, next) => {
  const { token } = c.req.param();
  const db = new DatabaseClient(c.env);
  const user = await db.findOne("users", { short_token: token, deleted_at: null });
  if (!user) return c.json({ error: "Not found" }, 404);
  c.set("userId", user.id);
  c.set("user", user);
  await next();
}, "validateShortLink");
app2.get("/u/:token", validateShortLink, async (c) => {
...
app2.get("/u/:token/entries", validateShortLink, async (c) => {
  const db = new DatabaseClient(c.env);
  ...
  const [entries, total] = await Promise.all([
    db.find("entries", filter, { orderBy: "date", order: "DESC", limit: pageSize > 0 ? pageSize : void 0, offset: pageSize > 0 ? offset : 0 }),
    db.count("entries", filter)
  ]);
  const formattedEntries = entries.map((e) => {
    let peakFlowReadings = [];
    try {
...
var createEntrySchema = external_exports.object({
  date: external_exports.string(),
  peakFlowReadings: external_exports.tuple([external_exports.number(), external_exports.number(), external_exports.number()]),
  spO2: external_exports.number().int().min(70).max(100),
  medicationTiming: external_exports.enum(["before", "after"]),
  period: external_exports.enum(["morning", "evening"]),
  note: external_exports.string().optional()
});
app2.post("/u/:token/entries", validateShortLink, zValidator("json", createEntrySchema), async (c) => {
  const db = new DatabaseClient(c.env);
  const userId = c.get("userId");
...
app2.get("/u/:token/export", validateShortLink, async (c) => {
  const db = new DatabaseClient(c.env);
  const userId = c.get("userId");
  const user = c.get("user");
  const entries = await db.find("entries", { user_id: userId }, { orderBy: "date", order: "ASC" });
...
app3.get("/admin/users", async (c) => {
  ...
  const [users, total] = await Promise.all([
    db.find("users", filter, { orderBy: "created_at", order: "DESC", limit: PAGE_SIZE2, offset }),
    db.count("users", filter)
  ]);
  const formattedUsers = await Promise.all(users.map(async (u) => {
    const lastEntry = await db.find("entries", { user_id: u.id }, { orderBy: "date", order: "DESC", limit: 1 });
    return {
      _id: u.id,
  ...
  return c.json({ users: formattedUsers, total, page, pageSize: PAGE_SIZE2 });
});
var createUserSchema = external_exports.object({
  ...
  personalBest: external_exports.number().int().min(50).max(900).nullable().optional(),
  adminNote: external_exports.string().optional()
});
app3.post("/admin/users", zValidator("json", createUserSchema), async (c) => {
  const db = new DatabaseClient(c.env);
  const data = c.req.valid("json");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const shortToken = crypto.randomUUID();
  const shortCode = Array.from(crypto.getRandomValues(new Uint8Array(4))).map((b) => b.toString(16).padStart(2, "0")).join("");
  const user = {
    id: crypto.randomUUID(),
...
app3.get("/admin/users/:id", async (c) => {
  const db = new DatabaseClient(c.env);
  const user = await db.findOne("users", { id: c.req.param("id") });
  if (!user) return c.json({ error: "Not found" }, 404);
  return c.json({
    _id: user.id,
    firstName: user.first_name,
...
var updateUserSchema = external_exports.object({
  ...
  nickname: external_exports.string().min(1).optional(),
  personalBest: external_exports.number().int().min(50).max(900).nullable().optional()
});
app3.patch("/admin/users/:id", zValidator("json", updateUserSchema), async (c) => {
  const db = new DatabaseClient(c.env);
  ...
  if (data.firstName !== void 0) updates.first_name = data.firstName;
  if (data.lastName !== void 0) updates.last_name = data.lastName;
  if (data.nickname !== void 0) updates.nickname = data.nickname;
  if (data.personalBest !== void 0) updates.personal_best = data.personalBest;
...
app3.delete("/admin/users/:id", async (c) => {
  const db = new DatabaseClient(c.env);
  const userId = c.req.param("id");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const user = await db.findOne("users", { id: userId });
...
app3.patch("/admin/users/:id/note", async (c) => {
  const db = new DatabaseClient(c.env);
  const userId = c.req.param("id");
...
app3.get("/admin/users/:id/export", async (c) => {
  const db = new DatabaseClient(c.env);
  const userId = c.req.param("id");
  ...
  const entries = await db.find("entries", filter, { orderBy: "date", order: "ASC" });
  let csv = "Date,Period,Best Peak Flow,SpO2,Medication,Note\n";
  for (const entry of entries) {
...
app3.get("/admin/entries", async (c) => {
  const db = new DatabaseClient(c.env);
  const page = parseInt(c.req.query("page") || "1");
  const userId = c.req.query("userId");
  const pageSize = c.req.query("pageSize") ? parseInt(c.req.query("pageSize")) : PAGE_SIZE2;
  const offset = pageSize > 0 ? (page - 1) * pageSize : 0;
  let filter = {};
  ...
  const formattedEntries = await Promise.all(entries.map(async (e) => {
    const user = await db.findOne("users", { id: e.user_id });
    let peakFlowReadings = [];
    try {
      peakFlowReadings = JSON.parse(e.peak_flow_readings || "[]");
    } catch {
  ...
  return c.json({ entries: formattedEntries, total, page, pageSize: PAGE_SIZE2 });
});
var updateEntrySchema = external_exports.object({
  date: external_exports.string().optional(),
  peakFlowReadings: external_exports.tuple([external_exports.number(), external_exports.number(), external_exports.number()]).optional(),
  spO2: external_exports.number().int().min(70).max(100).optional(),
  medicationTiming: external_exports.enum(["before", "after"]).optional(),
  period: external_exports.enum(["morning", "evening"]).optional(),
  note: external_exports.string().optional()
});
app3.patch("/admin/entries/:id", zValidator("json", updateEntrySchema), async (c) => {
  const db = new DatabaseClient(c.env);
  const entryId = c.req.param("id");
  const data = c.req.valid("json");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const entry = await db.findOne("entries", { id: entryId });
  if (!entry) return c.json({ error: "Not found" }, 404);
  const before = { ...entry };
  const updates = { updated_at: now };
  if (data.date !== void 0) updates.date = data.date;
  if (data.peakFlowReadings !== void 0) updates.peak_flow_readings = JSON.stringify(data.peakFlowReadings);
  if (data.spO2 !== void 0) updates.spo2 = data.spO2;
  if (data.medicationTiming !== void 0) updates.medication_timing = data.medicationTiming;
  if (data.period !== void 0) updates.period = data.period;
  if (data.note !== void 0) updates.note = data.note;
  ...
  const updated = await db.findOne("entries", { id: entryId });
  return c.json({
    _id: updated.id,
...
app3.delete("/admin/entries/:id", async (c) => {
  const db = new DatabaseClient(c.env);
  const entryId = c.req.param("id");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const entry = await db.findOne("entries", { id: entryId });
...
app3.get("/admin/audit", async (c) => {
  ...
  const [logs, total] = await Promise.all([
    db.find("audit_logs", filter, { orderBy: "timestamp", order: "DESC", limit: PAGE_SIZE2, offset }),
  ...
  return c.json({ logs: formattedLogs, total, page, pageSize: PAGE_SIZE2 });
});
var admin_default = app3;

// src/routes/redirect.ts
var app4 = new Hono2();
app4.get("/:code", async (c) => {
  const { code } = c.req.param();
  const db = new DatabaseClient(c.env);
  const user = await db.findOne("users", { short_code: code, deleted_at: null });
  if (!user) {
...
app5.route("/api", health_default);
app5.route("/api", user_default);
app5.route("/api", admin_default);
app5.route("/s", redirect_default);
...
// .wrangler/tmp/bundle-vRRkzo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vRRkzo/middleware-loader.entry.ts
