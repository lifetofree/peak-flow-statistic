...
#### ~~Issue #1: Business Logic in Route Handlers~~ ✅ Resolved
...
- **Resolution:** Created service layer with three new files:
  - `worker/src/services/entryService.ts` - Entry formatting, data building, and updates
  - `worker/src/services/userService.ts` - User formatting, token generation, and last entry date lookup
  - `worker/src/services/exportService.ts` - CSV generation and filename sanitization
  - Refactored all route handlers to use the service layer functions

#### ~~Issue #2: N+1 Query in User List with Last Entry Date~~ ✅ Resolved
- **Status:** Resolved (2026-04-14)
...
- **Resolution:** Added `DatabaseClient.getLastEntryDatesForUsers()` method using SQL `GROUP BY` with `MAX(date)` to efficiently fetch only the latest entry date per user. Updated `getLastEntryDates()` in userService to use the new method.
...
|------|-------|------------|
| 2026-04-14 | Business logic in route handlers | ✅ Extracted to service layer (entryService, userService, exportService) |
| 2026-04-14 | N+1 query in user list with last entry date | ✅ Optimized with SQL GROUP BY query |
...
- **Business logic in route handlers:** Resolved (2026-04-14) - extracted to service layer
